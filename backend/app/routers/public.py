...
from app.models import Girl, Game, TarotCard, TarotReading, HoroscopePrediction
...
async def register(data: RegisterIn, db: AsyncSession = Depends(get_db)):
    ...
    girl = Girl(
        name=name,
        email=f"guest-{uuid4()}@local",
        gift_certificate_url=None,
    )
    db.add(girl)
    await db.flush()
    await db.refresh(girl)
    token = create_access_token(girl.id)
    return RegisterOut(
        access_token=token,
        girl_id=girl.id,
        girl_name=girl.name,
